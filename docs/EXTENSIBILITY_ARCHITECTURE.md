...
GATalk 采用“模块化单体，预留插件能力”。当前不拆多进程服务，不开发动态
...
3. 预设项若写在主窗口中，会把 GATalk 业务配置和应用外壳绑定。
...
- 应用外壳新增工作台首页，只选择内置 Workspace，不承载业务状态或算法；
- `scenelens.visual_review` 继续使用 Project → Shot → Version；
- `scenelens.artwork_study` 使用独立单图研究包、审阅器、配置、Schema 和 UI；
- 两个模块只共享图像读取、画布、Provider、后台执行、凭据和通用工作区契约；
- 作品研究没有向场景项目数据库添加空表，也不依赖场景 Brief、区域或任务。

这证明第二个大型工作区可以接入而不改写场景业务模型。由于当前只有两个真实
工作区，首页卡片与窗口装配仍由可信内置代码显式完成；第三方动态加载继续推迟。
