...
M2 将 GATalk 从单一场景审阅模块扩展为可承载未来 Game Art Workbench
...
`Annotation`、`Task`、`ReviewProfile` 与 `QualityGate`。只有出现 GATalk
