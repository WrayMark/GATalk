...
function Assert-Python311X64 {
    ...
    if ($LASTEXITCODE -ne 0 -or $probe.Trim() -ne "3.11|64") {
        throw "GATalk requires Python 3.11 x64. Detected: $probe"
    }
}

if (-not (Test-Path -LiteralPath $venvPython)) {
    Write-Host "Creating the GATalk Python 3.11 virtual environment..."
    Assert-Python311X64
...
if ($needsInstall) {
    Write-Host "Installing pinned GATalk dependencies. The first run may take a few minutes..."
