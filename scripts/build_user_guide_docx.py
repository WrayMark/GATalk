...
def build() -> None:
    ...
    decimal_id = add_numbering(document, bullet=False)
    for text in (
        "在“清单与校正 → 拆分依据”选择视觉供应商，检查发送内容并主动确认。",
        "先检查空间层级、建筑或自然系统、重复规律、遮挡和不确定性，并写入人工修正。",
        ...
        "分析建议只是起点；用户可修改方案名称、用途、每类深度、分组方式、单页上限和备注。",
    ):
        add_list_item(document, text, decimal_id)
    add_heading(document, "最短流程", 2)
    decimal_id = add_numbering(document, bullet=False)
    ...
    for text in (
        ...
        add_list_item(document, text, bullet_id)
    add_heading(document, "三种工作方式", 2)
    for text in (
        "清单与校正：按当前方案生成结构化清单，再人工修改区域、分类、层级和复用关系。",
        "自动资产板：按当前方案批量生成单项概念图和分页展示板；结果不写入人工校正清单。",
