...
- 状态：Accepted
- 日期：2026-08-12
- 决策：主手册按“用途、入口、短步骤、结果、注意事项”组织；首页、全局设置和
  每个主要工作流至少配一张当前版本界面图。详细算法、架构和验证记录不堆入主手册。
- 决策：截图由可重复脚本使用合成项目生成，不含 API Key、私人路径或未授权作品；
  界面、术语或入口变化时必须更新受影响截图。
- 决策：简中和英文说明同步维护；Word 版由简中 Markdown 自动生成，并在交付前
  渲染逐页检查清晰度、分页、截断和图注。
